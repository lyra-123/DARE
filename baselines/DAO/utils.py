...
def calculate_marco_f1(gt_name, det_name, num, num_r, frame):
    ...
    gt_path = f'/mnt/mydisk/lyra/input/{gt_name}/{num}'
    det_path = f'/mnt/mydisk/lyra/input/{det_name}/{num_r}'
...
def calculate_macro_f1_conf_curve(
    gt_name,
    det_name,
    num,
    num_r,
    ...
    for conf_thr in conf_thresholds:
        ...
        for i in range(1, frame + 1):
            gt_file = os.path.join(gt_path, f"{num}_{i}.txt")
            det_file = os.path.join(det_path, f"{num}_{i}.txt")
            ...
            for box in read_detections(det_file, conf_thr):
                cls = int(box[0])
                all_pred_boxes[cls].append(box[1:])

            f1_scores = []
            for cls in all_gt_boxes:
                gt_boxes = all_gt_boxes[cls]
                pred_boxes = all_pred_boxes.get(cls, [])
                f1_scores.append(calculate_f1_for_class(gt_boxes, pred_boxes))

            frame_f1 = sum(f1_scores) / len(f1_scores) if f1_scores else 0
            all_frame_f1.append(frame_f1)

        macro_f1 = sum(all_frame_f1) / len(all_frame_f1) if all_frame_f1 else 0
        f1_curve.append(macro_f1)
    ...
    # # --------- 保存数值 ----------
    # txt_path = os.path.join(save_dir, "macro_f1_conf_result.txt")
    # with open(txt_path, "w") as f:
    #     f.write(f"Best F1: {best_f1:.6f}\n")
    #     f.write(f"Best confidence: {best_conf:.6f}\n")
    #
    # # --------- 保存 CSV ----------
    # csv_path = os.path.join(save_dir, "macro_f1_conf_curve.csv")
    # with open(csv_path, "w") as f:
    #     f.write("confidence,macro_f1\n")
    #     for c, f1 in zip(conf_thresholds, f1_curve):
    #         f.write(f"{c:.6f},{f1:.6f}\n")
    #
    # # --------- 画图并保存 ----------
    # plt.figure(figsize=(6, 4))
    # plt.plot(conf_thresholds, f1_curve, label="Macro-F1")
    # plt.scatter([best_conf], [best_f1], color='red')
    # plt.axvline(best_conf, linestyle='--')
    # plt.xlabel("Confidence threshold")
    # plt.ylabel("Macro-F1")
    # plt.title("Frame-level Macro-F1 vs Confidence")
    # plt.grid(True)
    # plt.legend()
    #
    # img_path = os.path.join(save_dir, "macro_f1_conf_curve.png")
    # plt.savefig(img_path, dpi=200, bbox_inches='tight')
    # plt.close()
    #
    # print(f"\n✔ Best Macro-F1 = {best_f1:.4f} @ conf = {best_conf:.3f}")
...
def read_detections(file_path, conf_thr):
    """
...
#
#     def cfg_id(r):
#         return qp_idx[r['BIT']] * (n_skip * n_re) + skip_idx[r['SKIP']] * n_re + re_idx[r['RE']]
...
def load_h5_file(h5_file, seq_min=0, seq_max=26):
    ...
    res = sorted(df['RE'].unique())
    n_re = len(res)
    bit_idx = {v: i for i, v in enumerate(bits)}
    re_idx = {v: i for i, v in enumerate(res)}

    def cfg_id(r):
        return bit_idx[r['BIT']] * n_re + re_idx[r['RE']]
