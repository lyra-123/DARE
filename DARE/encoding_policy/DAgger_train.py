...
def train_IL():
    ...
    logging.basicConfig(filename=LOG_FILE + f'/log_central_{NAME}', filemode='w', level=logging.INFO)

    with open(LOG_FILE + f'/log_test_{NAME}', 'w') as test_log_file:
        torch.manual_seed(RANDOM_SEED)
        model_IL = ImitationNet(C=C, k=S_LEN).type(dtype)
        model_IL.eval()
        ...
        for idx, h5_path in enumerate(h5_files):
            ...
            try:
                ...
                data_map = get_chunk_data_map(tmp_df, seq_min=start_idx, seq_max=start_idx + train_num - 1, seq_start=seq_train_start)
                ...
                data_map = get_chunk_data_map(tmp_df, seq_min=start_idx + train_num, seq_max=max_seq - 1, seq_start=seq_val_start)
                ...
                deg_train, deg_val, _, _ = load_deg_h5_as_chunk_map(deg_h5_path, seq_train_start=seq_train_start, seq_val_start=seq_val_start, train_num=train_num)
        ...
        while True:
            ...
            for seq_id in range(SEQ_TOTAL):
                ...
                p1_state = np.zeros((C, S_LEN + 1))
                p1_state = torch.from_numpy(p1_state)

                diff_state = np.zeros((C, S_LEN + 1))
                diff_state = torch.from_numpy(diff_state)
                ...
                while not end_of_video:
                    ...
                    p1_state = np.roll(p1_state, -1, axis=1)
                    p1_state[:, -1] = p1_feat
                    p1_state = torch.from_numpy(p1_state)

                    diff_state = np.roll(diff_state, -1, axis=1)
                    diff_state[:, -1] = diff_feat
                    diff_state = torch.from_numpy(diff_state)

                    p1_states.append(p1_state.unsqueeze(0))
                    diff_states.append(diff_state.unsqueeze(0))

                    with torch.no_grad():
                        qp_logits, s_logits, r_logits = model_IL(state.unsqueeze(0).type(dtype), p1_state.unsqueeze(0).type(dtype), diff_state.unsqueeze(0).type(dtype))

                        qp_prob = torch.softmax(qp_logits, dim=-1)
                        s_prob = torch.softmax(s_logits, dim=-1)
                        r_prob = torch.softmax(r_logits, dim=-1)

                        qp = int(qp_prob.multinomial(num_samples=1).item())
                        s = int(s_prob.multinomial(num_samples=1).item())
                        r = int(r_prob.multinomial(num_samples=1).item())
                    ...
                    bw, latency, _, size, f1, end_of_video = net_env.get_video_chunk(qp, s, r, encot_t)
                ...
                video = {
                    ...
                    'p1_states': torch.cat(p1_states, dim=0),
                    'diff_states': torch.cat(diff_states, dim=0),
                    'exp_actions': torch.cat(exp_actions, dim=0),
            ...
            for video in videos:
                ...
                p1_states = video['p1_states'].type(dtype)
                diff_states = video['diff_states'].type(dtype)
                exp_actions = video['exp_actions'].long().to(states.device)

                exp_qp = exp_actions // 25
                remainder = exp_actions % 25
                exp_s = remainder // 5
                exp_r = remainder % 5

                qp_logits, s_logits, r_logits = model_IL(states, p1_states, diff_states)

                loss_qp = criterion(qp_logits, exp_qp)
                loss_s = criterion(s_logits, exp_s)
                loss_r = criterion(r_logits, exp_r)
                loss = (loss_qp + loss_s + loss_r) / 3
            ...
            if epoch % 10 == 0:
                model_path = f'{OUTPUT_DIR}/IL_{epoch}.pth'
